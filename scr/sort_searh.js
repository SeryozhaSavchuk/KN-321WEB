function getArrayFromStorage() {
    ...
    for (let i = 0; i < keyNumbers; i++) {
        ...
        elm.id = keyName;
        elm.Model = row.Model;
        elm.Conect = row.Conect;
        elm.minhz = row.minhz;
        elm.maxhz = row.maxhz;
        elm.resistance = row.resistance;
...
function sortElements(){
    ...
    if (checkBox.checked == true){
       let sortArr = getArrayFromStorage()
        ...
        //Сортуємо масив об'єктів по полю Model
        sortArr.sort(byField('maxhz'));
...
function searchElements(){
  ...
  for (let i=0; i<searchtArr.length; i++) {
      ...
      let strC = row.Conect.toLowerCase();
      if (regExp.test(strM) || regExp.test(strC)) {buildElementToPage(row.id, row)
                                                                        isFounded = true}
      }
